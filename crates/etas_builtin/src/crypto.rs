...
pub fn constant_time_eq(args: &[BuiltinValue]) -> Result<BuiltinValue, BuiltinError> {
    expect_arity(args, 2)?;
    let (BuiltinValue::Bytes(lhs), BuiltinValue::Bytes(rhs)) = (&args[0], &args[1]) else {
        let actual = if !matches!(args[0], BuiltinValue::Bytes(_)) {
            args[0].type_tag()
        } else {
            args[1].type_tag()
        };
...
pub fn sha256_digest(args: &[BuiltinValue]) -> Result<BuiltinValue, BuiltinError> {
    expect_arity(args, 1)?;
    let BuiltinValue::Bytes(body) = &args[0] else {
        return Err(BuiltinError::TypeMismatch {
            expected: BuiltinTypeTag::Bytes,
